...
def build_pro_players(page: ft.Page, navigate) -> ft.Control:
        ...
        if p.get("photo_url"):
            photo_display = ft.Container(
                content=ft.Image(
                    src=p["photo_url"],
                    fit=ft.BoxFit.COVER,
                     error_content=ft.Text(p["team"][:1], weight=ft.FontWeight.BOLD, color=TEXT),
                    width=48, height=48,
                ),
            ...
            photo_display = ft.Container(
                content=ft.Text(p["team"][:1], size=20, weight=ft.FontWeight.BOLD, color=TEXT),
        ...
        return ft.Container(
            content=ft.Column([
                ft.Row([
                    photo_display,
                    ft.Column([
                        ft.Text(p["name"], size=16, weight=ft.FontWeight.BOLD, color=TEXT),
                        ft.Text(f"{p['team']} • {p['region']}", size=12, color=MUTED),
                    ], spacing=2, expand=True),
                    ft.Container(
                        content=ft.Image(
                            src=dd_service.get_role_icon_url(p["role"]), 
                            width=16, height=16, color=role_color
                        ),
                        padding=8, border_radius=10, bgcolor=f"{role_color}15"
                    )
                ], spacing=12),
                ft.Divider(height=1, color=BORDER),
                ft.Row([
                    ft.Row([
                        ft.Image(src=dd_service.get_champion_square_url(c), width=24, height=24, border_radius=12)
                        for c in p["champs"][:3]
                    ], spacing=4),
                    ft.Row([
                         ft.ElevatedButton(
                             ...
                             style=ft.ButtonStyle(
                                 ...
                                 side={ft.ControlState.DEFAULT: ft.BorderSide(1, f"{CYAN}40")},
    ...
    return ft.Container(
        content=ft.Column([
            ft.Row([
                 ft.Text("🏆 Pro Players", size=28, weight=ft.FontWeight.BOLD, color=GOLD),
            ...
            ft.Row([
                ...
                ft.Dropdown(
                    ...
                    on_select=refresh, bgcolor=CARD, border_color=BORDER
                ),
                ft.Dropdown(
